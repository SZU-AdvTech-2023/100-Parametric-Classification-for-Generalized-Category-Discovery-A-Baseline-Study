...
def k_core_set_greedy(inital_set, sampling_pool, sampling_num, batch_size=20000):
    ...
    with torch.no_grad():
        ...
        for cnt_sampling in range(sampling_num):
            ...
            excludeMask = np.ones(len(sampling_pool), dtype=np.bool_)
...
@torch.no_grad()
def optimize_loss_near_optimized(features_new, features_no_aug, features_train_no_aug,
                                 loss_weight, near_weight, save_dir, core_set_size, sampling_num=10000,
                                 write_txt_name="sampled.txt", losses=None, cloest_distance=None, idxs=None):
    ...
    if core_set_size > sampling_num:
        ...
        sampling_idx = idxs[sampling_idx]
        print(sampling_idx, '=================')
    else:
        sampling_idx = idxs[large_score_idx]

    # verify
    print("sampled loss with sampling idx is {}".format(np.sort(losses[large_score_idx])))
    print("sampled loss for large core set idx is {}".format(np.sort(losses[large_score_idx])))
    print(len(sampling_idx))
    # sample and write
    os.system("mkdir -p {}".format(save_dir))
    with open(join('', write_txt_name), 'w') as f:
...
@torch.no_grad()
def optimize_loss_near_normalized_togather_core(inference_root, split_save_dir,
    ...
    idxs_train_no_aug, labels_train_no_aug, _, features_train_no_aug_beforeProj = read_data_dir([join(inference_root, no_aug_sampling_dataset_inference), ],
                                                                                                map_location="cpu", return_before_proj_features=True)
    ...
    optimize_loss_near_optimized(features, features_no_aug_beforeProj, features_train_no_aug_beforeProj,
                                 loss_weight, 1-loss_weight, split_save_dir, core_sample_size,
                                 sampling_num=sampling_num, write_txt_name=loss_txt_name, losses=losses,
                                 cloest_distance=cloest_distance, idxs=idxs[0])
...
def main():
    ...
    parser.add_argument('--core_sample_ratio', default=2.0, type=float)
    parser.add_argument('--sampling_num', default=1000, type=int)
    parser.add_argument('--loss_repeat_time', default=5, type=int)

    args = parser.parse_args()
    args.loss_txt_name = 'test_result/1/loss.txt'
    args.loss_dist_save_dir = 'test_result/1/'
    args.inference_root = 'test_result/'
    args.no_aug_sampling_dataset_inference_exp = ''
    args.aug_sampling_dataset_inference_exp = ''


    optimize_loss_near_normalized_togather_core(args.inference_root, args.split_save_dir,
